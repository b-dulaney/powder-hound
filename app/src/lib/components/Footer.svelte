<div class="bg-surface-50 dark:bg-surface-800 w-full p-6">
	<div class="mx-auto max-w-screen-xl">
		<div class="md:flex md:justify-between">
			<div class="mb-6 md:mb-0">
				<a href="/" class="flex items-center">
					<enhanced:img src="/src/public/logo.png" class="mr-3 h-10 w-10" alt="PowderHound Logo" />
					<span class="dark:text-white self-center whitespace-nowrap text-2xl font-semibold"
						>PowderHound</span
					>
				</a>
			</div>
			<div class="grid grid-cols-2 gap-8 sm:gap-6">
				<div>
					<h2 class="dark:text-white mb-6 text-sm font-semibold uppercase text-gray-900">
						Resources
					</h2>
					<ul class="dark:text-gray-400 text-gray-600">
						<li class="mb-4">
							<a href="https://avalanche.state.co.us/" class="hover:underline">CAIC</a>
						</li>
						<li class="mb-4">
							<a href="https://github.com/b-dulaney/powder-hound" class="hover:underline">Github</a>
						</li>
					</ul>
				</div>
				<div>
					<h2 class="dark:text-white mb-6 text-sm font-semibold uppercase text-gray-900">Legal</h2>
					<ul class="dark:text-gray-400 text-gray-600">
						<li class="mb-4">
							<a href="/privacy-policy" class="hover:underline">Privacy Policy</a>
						</li>
						<li>
							<a href="/terms-of-use" class="hover:underline">Terms of Use</a>
						</li>
					</ul>
				</div>
			</div>
		</div>
		<hr class="dark:border-gray-700 my-6 border-gray-200 sm:mx-auto lg:my-8" />
		<span class="dark:text-gray-400 inline-flex items-center gap-2 py-2 text-gray-600">
			<svg
				class="h-6 w-6"
				aria-hidden="true"
				xmlns="http://www.w3.org/2000/svg"
				width="24"
				height="24"
				fill="currentColor"
				viewBox="0 0 24 24"
			>
				<path
					d="M2.038 5.61A2.01 2.01 0 0 0 2 6v12a2 2 0 0 0 2 2h16a2 2 0 0 0 2-2V6c0-.12-.01-.238-.03-.352l-.866.65-7.89 6.032a2 2 0 0 1-2.429 0L2.884 6.288l-.846-.677Z"
				/>
				<path
					d="M20.677 4.117A1.996 1.996 0 0 0 20 4H4c-.225 0-.44.037-.642.105l.758.607L12 10.742 19.9 4.7l.777-.583Z"
				/>
			</svg>
			<p>contact@powderhound.io</p>
		</span>
	</div>
</div>
